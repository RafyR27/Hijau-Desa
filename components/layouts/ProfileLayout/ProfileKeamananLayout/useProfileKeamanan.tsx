...
type FormSetPasswordValues = z.infer<typeof formSetPasswordSchema>;
...
export const useProfileKeamanan = ({ user }: { user?: SessionUser }) => {
  ...
  const {
    ...
    isPending: isPendingSetPassword,
  } = useMutation({
    mutationFn: setPasswordService,
    onError(error: Error) {
      toast.error(error?.message || "Gagal membuat password", {
        position: "top-right",
      });
    },
    onSuccess: () => {
